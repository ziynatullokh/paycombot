...
async function createChek (account,account_type,amount,merchant){
    try{
        amount *= 100
...
export default async function (msg,user,order,newCard = true){
    ...
    let chequeCreate = await createChek(order.order_account, order.order_account_type, order.order_amount, order.order_merchant)
    ...
    user.session = 'inputVerifyCodeCommon'
    user.save()
