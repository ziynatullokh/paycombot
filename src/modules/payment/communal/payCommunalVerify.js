...
export default async function (msg,user){
    ...
    let chequeVerify = await chequePay(order.order_chek, order.order_cardnumber, order.order_cardexpire, text)
    ...
    if(chequeVerify == 'success') {
        ...
        const keyboard = [
            ...
            [
                {
                    text: dt[user.language]['repeatCheque'] , callback_data: 'repeatChequeCommunal-' + order.order_chek
                },
                {
